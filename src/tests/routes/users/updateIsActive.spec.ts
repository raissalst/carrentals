...
describe('update isActive users route tests', () => {
  ...
  it('401, should not update an user without token', async () => {
    const response = await request(app).patch(`/api/users/${userMock.id}`);

    expect(response.statusCode).toBe(401);
  });

  it('204, should update user with an admin token', async () => {
  ...
  it('401, should not update an user with a customer token', async () => {
    const response = await request(app)
      .patch(`/api/users/${userMock.id}`)
  ...
  it('404, should not update an user with a wrong user id', async () => {
    const response = await request(app)
      .patch(`/api/users/${v4()}`)
      .set('Authorization', `Bearer ${adminToken}`);
  ...
  it('204, should deactivate company with an admin token', async () => {
    const response = await request(app)
  ...
  it('204, should activate company with an admin token', async () => {
    const response = await request(app)
      .patch(`/api/users/${company.id}`)
      .set('Authorization', `Bearer ${adminToken}`);
